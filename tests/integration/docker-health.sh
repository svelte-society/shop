...
start_container() {
	...
	if [[ "$scheduler" == true ]]; then
		arguments+=(
			...
			--env PLUNK_BASE_URL=https://next-api.useplunk.com
...
# The same volume becomes ready only after bootstrap is turned off. This run enables
# the real scheduler so SIGTERM proves scheduler drain precedes SQLite close.
...
docker exec "$NORMAL_CONTAINER" node --input-type=module --eval '
	import Database from "better-sqlite3";
	const database = new Database("/data/shop.sqlite");
...
assert_shutdown_logs "$NORMAL_CONTAINER" true
